...
function parseArgs() {
    ...
    for (let i = 0; i < args.length; i++) {
        const a = args[i];
        if (a === '--mongodb-uri')
            out.mongodbUri = args[++i];
        else if (a?.startsWith('--mongodb-uri='))
            out.mongodbUri = a.split('=')[1];
        else if (a === '--admin-email')
            out.adminEmail = args[++i];
        else if (a === '--admin-password')
            out.adminPassword = args[++i];
        else if (a === '--admin-name')
            out.adminName = args[++i];
        else if (a === '--drop-vector-index')
            out.dropVectorIndex = true;
...
async function main() {
        ...
        for (const [, Model] of Object.entries(models)) {
            ...
            if (m && typeof m.createIndexes === 'function') {
                try {
                    ...
                    console.warn(`  ! ${m.modelName ?? 'model'}: ${err.message}`);
                }
            }
    ...
    await step('Seed default Badges', async () => {
        const { Badge } = (await loadAllModels());
    ...
    await step('Seed default SupportCategories', async () => {
        const { SupportCategory } = (await loadAllModels());
        const cats = [
            { issueType: 'internet', label: 'Internet Problem', shortLabel: 'Internet', description: 'Wi-Fi, hotspot, or general connectivity issues', iconKey: 'wifi',
            ...
            { issueType: 'camera', label: 'Camera Issue', shortLabel: 'Camera', description: 'Camera permission, hardware, or app-level issues', iconKey: 'camera',
            ...
            { issueType: 'microphone', label: 'Microphone Issue', shortLabel: 'Microphone', description: 'Mic permission, hardware, or app-level issues', iconKey: 'mic',
            ...
            { issueType: 'device', label: 'Device Failure', shortLabel: 'Device', description: 'Laptop / phone hardware failures', iconKey: 'device',
            ...
            { issueType: 'power', label: 'Power Outage', shortLabel: 'Power', description: 'Power cuts, battery, or charging issues', iconKey: 'power',
            ...
            { issueType: 'other', label: 'Other Reason', shortLabel: 'Other', description: 'Anything not covered by the categories above', iconKey: 'generic',
    ...
    await step('Create initial admin user', async () => {
        const { User } = (await loadAllModels());
    ...
    if (args.skipAudit) {
        ...
        try {
            ...
            console.warn(`  ! audit:data exited with code ${err.status} — review above.`);
        }
    }
    console.log('\n=========================');
