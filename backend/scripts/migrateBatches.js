...
import Category, { slugifyCategoryName } from '../models/Category.js';
import FAQ from '../models/FAQ.js';
const LEGACY_BATCH_NAME = 'Yaksha 2025–26';
const LEGACY_BATCH_DESCRIPTION = 'Pre-batch-migration cohort. Holds all FAQs created before the Batch ' +
...
async function main() {
    ...
    const legacy = await Batch.findOneAndUpdate({ name: LEGACY_BATCH_NAME }, {
        $setOnInsert: {
    ...
    for (const rawName of distinct) {
        ...
        const existing = await Category.findOneAndUpdate({ batchId: legacy._id, slug }, {
        ...
        if (existing.createdAt.getTime() < Date.now() - 1000)
            catReused += 1;
        else
            catCreated += 1;
    }
    ...
    if (needsCatId === 0) {
        ...
        const cursor = FAQ.find({ batchId: legacy._id, categoryId: null }, { _id: 1, category: 1 }).cursor();
        const ops = [];
        for await (const f of cursor) {
            ...
            if (!catId)
                continue;
            ops.push({
                updateOne: { filter: { _id: f._id }, update: { $set: { categoryId: catId } } },
            });
        }
        if (ops.length > 0) {
