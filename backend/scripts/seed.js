...
const DEFAULT_BATCH = {
    name: 'Yaksha 2026-27',
    description: 'The two-month, full-time research internship at the Vicharanashala Lab, ' +
...
const seed = async () => {
    try {
        ...
        try {
            ...
            const allFaqs = Array.isArray(parsed)
                ? parsed
                : (parsed.faqs ?? []);
            ...
            if (orphaned > 0) {
                const res = await FAQ.updateMany({ batchId: null }, { $set: { batchId: defaultBatch._id } });
                console.log(`  ✓ Backfilled ${res.modifiedCount} orphaned FAQ(s) to default batch`);
            }
        }
        catch (err) {
            console.warn(`  ⚠ Warning: Could not read faqs.json from ${faqPath}. Skipping FAQ seeding. ${err.message}`);
        }
