...
async function main() {
    ...
    for await (const doc of cursor) {
        const oldTier = doc.tier;
        const newTier = TIER_MAP[oldTier];
        if (!newTier)
            continue;
        try {
            const result = await collection.updateOne({ _id: doc._id }, { $set: { tier: newTier } });
    ...
    for (const [oldTier, newTier] of Object.entries(TIER_MAP)) {
        const result = await logCollection.updateMany({ 'metadata.tier': oldTier }, { $set: { 'metadata.tier': newTier } });
        if (result.modifiedCount > 0) {
