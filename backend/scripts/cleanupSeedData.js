...
async function main() {
    ...
    //    of the array intact.
    const t4 = await db.collection('yaksha_faq_users').updateMany({ 'positiveBadges.reason': SEED_BADGE_REASON }, { $pull: { positiveBadges: { reason: SEED_BADGE_REASON } } });
    ...
    const oldest = await db.collection('yaksha_faq_users')
        ...
        .limit(8)
        .toArray();
    const oldestIds = oldest.map((u) => u._id);
    const t5 = await db.collection('yaksha_faq_users').updateMany({ _id: { $in: oldestIds } }, { $set: { points: 0, reputation: 0, tier: 'newcomer', acceptedAnswers: 0, faqContributions: 0 } });
