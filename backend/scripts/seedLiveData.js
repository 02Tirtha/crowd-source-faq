...
async function seedCommunityPosts(db) {
    ...
    if (users.length === 0) {
        console.log('  no users, skipping community posts');
        return;
    }
...
async function seedSupportTickets(db) {
    ...
    if (users.length === 0) {
        console.log('  no users, skipping support tickets');
        return;
    }
    ...
    for (let i = 0; i < toCreate; i++) {
        ...
        const statusHistory = [
            { status: 'Pending', note: 'Ticket created', updatedBy: requester._id, updatedByName: requester.name, timestamp: daysAgo(t.daysAgo) },
        ];
        if (t.status !== 'Pending') {
            statusHistory.push({
                status: t.status,
                note: `Moved to ${t.status} by admin`,
                updatedBy: (await User.findOne({ role: 'admin' }).select('_id').lean())?._id ?? requester._id,
                updatedByName: 'admin',
        ...
        await SupportRequest.create({
            ...
            userEmail: `${requester.name.toLowerCase().replace(/[^a-z0-9]/g, '.')}@yaksha.com`,
...
async function seedBadges(db) {
    ...
    if (badges.length === 0) {
        console.log('  no badges defined, run seed:badges first');
        return;
    }
    const users = await User.find({}).select('_id').lean();
    if (users.length === 0)
    ...
    for (let i = 0; i < Math.min(10, users.length); i++) {
        ...
        if (!positiveBadge)
            continue;
        await db.collection('yaksha_faq_users').updateOne({ _id: u._id }, {
...
async function seedZoomMeetings(db) {
...
async function seedDocumentRecords(db) {
...
async function seedLeaderboardPoints(db) {
...
async function seedSearchLogs(db) {
