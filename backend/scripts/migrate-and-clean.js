...
const userResult = await db.collection(USERS).updateMany({ $or: [
...
for (const [oldTier, newTier] of Object.entries(TIER_MAP)) {
    const r = await db.collection(USERS).updateMany({ tier: oldTier }, { $set: { tier: newTier } });
    if (r.modifiedCount > 0)
        log(`  ${oldTier} → ${newTier}: ${r.modifiedCount} users`);
}
...
if (postMissing > 0) {
    ...
    for await (const post of cursor) {
        ...
        await db.collection(POSTS).updateOne({ _id: post._id }, { $set: {
                lifecycle: {
...
const faqResult = await db.collection(FAQS).updateMany({ tags: { $exists: false } }, { $set: { tags: [] } });
...
async function ensureIndex(col, keys, opts = {}) {
    try {
