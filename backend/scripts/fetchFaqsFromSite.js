...
async function main() {
    ...
    try {
      ...
      return out;
        ...
        let oldVersion = '<none>';
        try {
            const old = JSON.parse(await fs.readFile(OUTPUT_PATH, 'utf-8'));
            ...
            if (added.length)
                console.log(`    added: ${added.slice(0, 10).join(', ')}${added.length > 10 ? '…' : ''}`);
            if (removed.length)
                console.log(`    removed: ${removed.slice(0, 10).join(', ')}${removed.length > 10 ? '…' : ''}`);
        }
        catch {
