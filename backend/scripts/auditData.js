...
const issues = [];
function flag(collection, kind, detail) {
...
async function getUserIdSet(db) {
    const userIds = await db.collection('yaksha_faq_users').distinct('_id');
    return new Set(userIds.map((id) => idToString(id)).filter((s) => s !== null));
}
async function auditUsers(db) {
    ...
    if (noBanner > 0)
        flag('User', 'banned-without-bannedBy', `${noBanner} users have isBanned=true but no bannedBy`);
...
async function auditFAQs(db) {
    ...
    for (const f of allFaqs) {
        ...
        if (f.embedding.length !== EMBEDDING_DIM)
            wrongDim++;
    }
...
async function auditCommunityPosts(db) {
    console.log('\n=== Community Posts ===');
    ...
    for (const p of allPosts) {
        if (!p.embedding || p.embedding.length === 0)
            continue;
        if (p.embedding.length !== EMBEDDING_DIM)
            wrongDim++;
    }
    if (wrongDim > 0) {
...
async function auditSupportRequests(db) {
    console.log('\n=== Support Requests ===');
...
async function auditNotifications(db) {
    console.log('\n=== Notifications ===');
...
async function auditSearchLogs(db) {
    console.log('\n=== Search Logs ===');
