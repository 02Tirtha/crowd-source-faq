...
async function main() {
    ...
    if (!defaultBatch) {
        console.error('No default batch');
        process.exit(1);
    }
    console.log(`Default batch: "${defaultBatch.name}" (${defaultBatch._id})`);
    console.log(`  isActive: ${defaultBatch.isActive}`);
    console.log(`  enrollmentMode: ${defaultBatch.enrollmentMode}`);
    console.log(`  startDate: ${defaultBatch.startDate}`);
    console.log(`  endDate: ${defaultBatch.endDate}\n`);
    // ProgramSettings
    ...
    for (const b of allBatches) {
        console.log(`  - "${b.name}"  isDefault=${b.isDefault}  isActive=${b.isActive}  enrollmentMode=${b.enrollmentMode}`);
    }
