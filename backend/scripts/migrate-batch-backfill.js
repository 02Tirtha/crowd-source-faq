...
const TARGETS = [
    ...
    { label: 'GuestEvent', model: GuestEvent },
    { label: 'CommunityPost', model: CommunityPost },
    { label: 'ZoomMeeting', model: ZoomMeeting },
    { label: 'DocumentInsight', model: DocumentInsight },
    { label: 'TranscriptKnowledge', model: TranscriptKnowledge },
    { label: 'Badge', model: Badge },
    { label: 'ReputationLog', model: ReputationLog },
    { label: 'SearchLog', model: SearchLog },
    { label: 'UnresolvedSearch', model: UnresolvedSearch },
    { label: 'Notification', model: Notification },
    { label: 'TeaNotification', model: TeaNotification },
    { label: 'SupportRequest', model: SupportRequest },
    { label: 'AiQuestion', model: AiQuestion },
];
async function main() {
    ...
    if (!defaultBatch) {
        console.error('ERROR: no default batch found. Run `npm run seed` first ' +
    ...
    for (const { label, model } of TARGETS) {
        ...
        const res = await model.updateMany({ batchId: null }, { $set: { batchId: defaultBatch._id } });
    ...
    const aiConfigResult = await AiConfig.updateMany({ batchId: { $exists: false } }, { $set: { batchId: null } });
    ...
    const featureFlagResult = await FeatureFlag.updateMany({ batchId: { $exists: false } }, { $set: { batchId: null } });
    ...
    const enrollmentModeResult = await Batch.updateMany({ enrollmentMode: { $exists: false } }, { $set: { enrollmentMode: 'open' } });
