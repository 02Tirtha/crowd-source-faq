...
async function main() {
    ...
    // ── ZoomMeeting ────────────────────────────────────────────────────────────
    console.log('\n=== ZoomMeeting ===');
    const mtgResult = await db.collection('yaksha_zoom_meetings').updateMany({ sourcing: { $exists: false } }, {
        $set: {
            sourcing: 'webhook',
            sourceType: 'zoom',
    ...
    // ── ZoomInsight ────────────────────────────────────────────────────────────
    console.log('\n=== ZoomInsight ===');
    const insResult = await db.collection('yaksha_zoom_insights').updateMany({ sourcing: { $exists: false } }, {
        $set: {
            sourcing: 'webhook',
            processedBy: 'unknown:unknown',
