...
export function viewComponent(guestViewCount) {
    if (!guestViewCount || guestViewCount <= 0)
        return 0;
    return Math.min(1, Math.log10(1 + guestViewCount) / VIEW_LOG_REFERENCE);
}
/** Exponential decay by age in days. */
export function recencyComponent(createdAt) {
    if (!createdAt)
        return 0;
...
export function engagementComponent(avgReadCompletion, avgTimeSpentRatio) {
...
export function trustComponent(level) {
    switch (level) {
 ...
 */
export function buildScoreExpression(fieldPrefix) {
