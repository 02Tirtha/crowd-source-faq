...
function getClientId() { const v = process.env.ZOOM_CLIENT_ID; if (!v)
    throw new Error('Missing ZOOM_CLIENT_ID env var — add it to backend/.env.local'); return v; }
function getClientSecret() { const v = process.env.ZOOM_CLIENT_SECRET; if (!v)
    throw new Error('Missing ZOOM_CLIENT_SECRET env var — add it to backend/.env.local'); return v; }
function getRedirectUri() { return process.env.ZOOM_REDIRECT_URI ?? 'http://localhost:6767/api/zoom/auth/callback'; }
export async function getProgramZoomConfig(batchId = null) {
    if (batchId) {
        try {
 ...
 */
export function buildDynamicRedirectUri(request) {
...
function getStateSecret() {
    ...
    if (!v)
        throw new Error('OAUTH_STATE_SECRET (or legacy JWT_SECRET) required to sign OAuth state');
    return v;
}
/** Sign a state token for the given user. Returns the encoded `state` param. */
export function signOAuthState(internalUserId) {
...
export function verifyOAuthState(state) {
    try {
        ...
        logger.warn(`[zoomOAuth] State verification failed for state token: ${err.message}`);
        return null;
    }
}
/**
 ...
 */
export async function buildZoomAuthUrl(internalUserId, request, batchId = null) {
 ...
 */
export async function exchangeCodeForTokens(code, batchId = null) {
    const cfg = await getProgramZoomConfig(batchId);
 ...
 */
export async function refreshZoomTokens(refreshToken, batchId = null) {
    const cfg = await getProgramZoomConfig(batchId);
    return await zoomOAuthCircuit.execute(async () => {
        const credentials = Buffer.from(`${cfg.clientId}:${cfg.clientSecret}`).toString('base64');
 ...
 */
export async function getUserZoomToken(userId) {
    ...
    if (isExpired) {
        if (!refreshToken)
            throw new Error('No refresh token — user needs to reconnect Zoom');
 ...
 */
export async function getZoomUserId(accessToken) {
    ...
    if (!res.ok)
        throw new Error(`Failed to get Zoom user info (${res.status})`);
    const data = await res.json();
    // Return id from /users/me or first user from list
    const zoomUserId = data.id ?? data.users?.[0]?.id;
    if (!zoomUserId)
        throw new Error('Could not extract Zoom user ID from response');
    return zoomUserId;
}
/**
 ...
 */
export async function zoomApiAsUser(userId, path, options = {}) {
 ...
 */
export async function zoomApiWithFallback(userId, path, options = {}) {
    const { zoomFallback } = await import('./zoomFallback.js');
    return zoomFallback.withFallback({
        cacheKey: `zoom:${userId}:${path}`,
        cacheTtlMs: 60_000,
        fetch: () => zoomApiAsUser(userId, path, options),
    });
}
/**
 * Download a transcript file using a user's stored token.
 */
export async function downloadTranscriptAsUser(userId, downloadUrl) {
    const token = await getUserZoomToken(userId);
 ...
 */
export async function getPastRecordings(userId, from, to, pageSize = 50) {
    const toDate = to ?? new Date().toISOString().split('T')[0];
    const fromDate = from ?? new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
    const data = await zoomApiAsUser(userId, `/users/me/recordings?from=${fromDate}&to=${toDate}&page_size=${Math.min(pageSize, 300)}`);
