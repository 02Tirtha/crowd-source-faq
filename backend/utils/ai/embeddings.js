 ...
 */
async function callHfApiEmbedding(text) {
...
async function getEmbedder() {
    if (!cachedEmbedder) {
        ...
        cachedEmbedder = await pipeline('feature-extraction', MODEL_SLUG, { dtype: 'fp32' });
        isWarmed = true;
    }
    return cachedEmbedder;
}
/** Warm up the in-process embedding pipeline (no-op if using API). */
export const warmEmbedder = async () => {
    if (shouldUseHfApi())
...
export const __isWarmed = () => isWarmed;
