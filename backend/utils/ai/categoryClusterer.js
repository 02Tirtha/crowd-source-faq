 ...
 */
async function buildCategoryCentroids(batchId) {
    ...
    for (const n of names) {
        try {
            ...
            logger.warn(`[categoryClusterer] embed failed for "${n.name}": ${err.message}`);
            vectors.push(null);
        }
    }
    const out = [];
 ...
 */
function clusterByDotProduct(categories) {
    const clusters = [];
    for (const cat of categories) {
        ...
        if (bestIdx === -1) {
            ...
            const merged = c.centroid.map((v, i) => (v * c.faqCount + cat.centroid[i] * cat.faqCount) / newCount);
 ...
 */
async function aiNameCluster(aliases) {
    if (aliases.length === 1)
        return aliases[0];
    ...
    try {
        const reply = await chatWithProvider('anthropic', [{ role: 'user', content: prompt }], anthropicModel);
        ...
        logger.warn(`[categoryClusterer] AI naming failed for [${aliases.join(', ')}]: ${err.message}`);
        return fallback;
    }
}
/**
 ...
 */
function pickFallbackName(aliases) {
 ...
 */
async function persistClusters(batchId, clusters) {
    ...
    const aliasCountMap = new Map();
    for (const c of clusters) {
        for (const a of c.aliases)
            aliasCountMap.set(a, c.faqCount / c.aliases.length);
    }
 ...
 */
export async function clusterCategoriesForBatch(batchId, opts = {}) {
    ...
    if (categories.length === 0) {
        if (!opts.dryRun)
            await CategoryCluster.deleteMany({ batchId, locked: { $ne: true } });
    ...
    logger.info(`[categoryClusterer] batch ${batchId}: ${inserted} clusters inserted, ${preserved} locked clusters preserved (input: ${categories.length} categories)`);
    return { clusters: inserted, preservedLocks: preserved, skipped: 'ok' };
}
/**
 * Recompute for every active batch. Called by the 24h cron.
 */
export async function clusterAllActiveBatches() {
    ...
    for await (const b of cursor) {
        try {
            ...
            logger.error(`[categoryClusterer] failed for batch ${b._id}: ${err.message}`);
        }
    }
}
// ─── Math helpers ──────────────────────────────────────────────────────
