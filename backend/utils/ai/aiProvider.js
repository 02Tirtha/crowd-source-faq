 ...
 */
export function resolvePipelineProvider(pipeline) {
    const override = PIPELINE_PROVIDER_KEY[pipeline];
    if (override && isValidProvider(override))
        return override;
 ...
 */
export function resolvePipelineModel(pipeline, provider) {
...
function isValidProvider(p) {
    return PROVIDER_DEFAULTS[p] !== undefined;
}
/**
 * Map model names to resolved provider defaults in case of provider mismatch.
 */
export function getModelForProvider(model, provider) {
    const defaults = {
...
export async function getPipelineProviderConfig(pipeline, batchId = null) {
    const db = await resolveActiveAiConfig(batchId) ?? await loadDbOverrides();
    const hasKey = (p) => !!(db[p].apiKey || process.env[ENV_KEY[p]]);
    let provider;
    const override = PIPELINE_PROVIDER_KEY[pipeline];
    if (override && isValidProvider(override) && hasKey(override)) {
        ...
        try {
            ...
            logger.warn(`[aiProvider] Failed to find active AiConfig in getPipelineProviderConfig: ${err.message}`);
        }
        if (dbActive && hasKey(dbActive)) {
            ...
            if (hasKey('anthropic'))
                ...
                provider = 'xai';
            else if (hasKey('minimax'))
                provider = 'minimax';
            else if (hasKey('gemini'))
                provider = 'gemini';
    ...
    const apiKey = (db[provider].apiKey || process.env[keyEnv] || '');
...
export async function resolveActiveAiConfig(batchId = null) {
    ...
    try {
        ...
        logger.warn(`[aiProvider] resolveActiveAiConfig failed: ${err.message}`);
 ...
 */
export async function resolveProviderAsync(provider) {
...
export function resolveProvider() {
    ...
    throw new Error('No AI API key configured. Set one of:\n' +
...
export async function hasAIKeyAsync() {
    const db = await loadDbOverrides();
    return !!(db.anthropic.apiKey || process.env.ANTHROPIC_API_KEY ||
 ...
 */
export async function getProviderAsync(provider) {
 ...
 */
export async function chatWithProvider(provider, messages, model) {
 ...
 */
export async function chatWithConfig(config, messages) {
    const { provider, baseURL, apiKey, model, authHeader, needsAnthropicVersion } = config;
    if (!apiKey)
        throw new Error(`No API key for provider '${provider}' — set ${provider.toUpperCase()}_API_KEY`);
    if (provider === 'anthropic') {
